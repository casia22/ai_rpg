...
class NPCEngine:
                ...
                try:
                    ...
                    if "func" in json_data.keys():
                        ...
                        if hasattr(self, func_name):
                            ...
                            loop.run_until_complete(func(json_data))
                        # test
    ...
    async def create_conversation(self, json_data):
        ...
        # 初始化群体描述、心情和记忆
        descs: List[str] = [npc.desc for npc in npc_refs] + [json_data["player_desc"]] ###########################反了
    ...
    async def confirm_conversation_line(self, json_data):
        ...
        if conversation_id in self.conversation_dict:
            convo = self.conversation_dict[conversation_id]
            judge = convo.add_temp_memory(conversation_id, index)##################修改原因：convo中无add_memory
            if judge:
                self.npc_add_memory(convo)#############################修改原因：同上
