...
public class mm : MonoBehaviour
{
    ...
    void MoveCtrl(float m_speed)
    {
        ...
        if (animator) animator.SetInteger("girl_move", -1);
        if (Input.GetKey(KeyCode.W))
        {
            sum += 1;
            vec += Vector3.up * m_speed * Time.deltaTime;
            if (animator) animator.SetInteger("girl_move", 0);
        }
        ...
        if (Input.GetKey(KeyCode.A))
        {
            ...
            if (animator) animator.SetInteger("girl_move", 2);
        }
        if (Input.GetKey(KeyCode.D))
        {
            sum += 1;
            vec += Vector3.right * m_speed * Time.deltaTime;
            if (animator) animator.SetInteger("girl_move", 3);
        }
